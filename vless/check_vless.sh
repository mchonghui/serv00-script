...
save_config() {
    local port=$1
    if [[ ! -f ~/domains/$USER.cloudns.ch/vless/config.json ]]; then
        uuid=$(generate_uuid)
        cat <<EOL > ~/domains/$USER.cloudns.ch/vless/config.json
{
    "uuid": "$uuid",
    "port": $port
}
EOL
        ...
        jq --arg port "$port" '.port = ($port | tonumber)' ~/domains/$USER.cloudns.ch/vless/config.json > ~/domains/$USER.cloudns.ch/vless/config_tmp.json && mv ~/domains/$USER.cloudns.ch/vless/config_tmp.json ~/domains/$USER.cloudns.ch/vless/config.json
...
deploy_vless() {
    ...
    npm install
    # 启动vless项目
    ~/.npm-global/bin/pm2 start ~/domains/$USER.cloudns.ch/vless/app.js --name vless
    # 保存pm2进程状态
    ...
    echo -e "域名: ${GREEN}$USER.cloudns.ch${NC}"
    echo -e "vless进程维护定时任务脚本: ${GREEN}cd ~/domains/$USER.cloudns.ch/vless && ./check_vless.sh${NC}"
    echo -e "VLESS节点信息: ${GREEN}vless://${uuid}@$USER.cloudns.ch:${port}?flow=&security=none&encryption=none&type=ws&host=$USER.cloudns.ch&path=/&sni=&fp=&pbk=&sid=#$USER.cloudns.vless${NC}"
}
...
main() {
    ...
    if [ "$port_provided" = true ]; then
        ...
        if [[ -f config.json ]]; then
            ...
            echo -e "UUID: ${uuid}"
            echo -e "Port: ${port}"
            echo -e "域名: $USER.cloudns.ch"
            echo -e "VLESS节点信息: vless://${uuid}@$USER.cloudns.ch:${port}?flow=&security=none&encryption=none&type=ws&host=$USER.cloudns.ch&path=/&sni=&fp=&pbk=&sid=#$USER.cloudns.vless"
